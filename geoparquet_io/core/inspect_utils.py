...
def wkb_to_wkt(wkb_bytes: bytes) -> Optional[str]:
    """
    Convert WKB binary data to WKT string representation using DuckDB.

    Args:
        wkb_bytes: WKB binary data

    Returns:
        str: WKT string representation, or None if conversion fails
    """
    if not wkb_bytes:
        return None
...
def format_geometry_display(value: Any) -> str:
    ...
    if isinstance(value, bytes):
        # Try to convert WKB to WKT using DuckDB
        wkt = wkb_to_wkt(value)
        if wkt:
            return wkt
        # Fall back to type-only display if conversion fails
