...
| `--row-group-size` | 49,152 | Row count per group, snapped to the nearest whole 2,048-row writer vector (10k-50k recommended for spatial pushdown) |
...
row-group-sized tiles, but both uses take the same value - snapped to the
nearest whole 2,048-row writer vector - so strips end on row-group boundaries
