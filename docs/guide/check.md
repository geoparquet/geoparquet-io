...
!!! tip "Spatial filter pushdown and row group sizing"
    For GeoParquet 2.0 or parquet-geo-only files with Hilbert sorting, row groups of 10,000-50,000 rows create tighter bounding boxes that enable more row group skipping during spatial queries. Re-sorting nine published catalog files from 100,000 to 50,000 rows per group cut the share of the file a query window covering 10% of each dimension has to read from 43-100% down to 10-28% ([#775](https://github.com/geoparquet/geoparquet-io/issues/775)). The `gpio sort` commands ask for 50,000 rows per group, the top of that band. DuckDB rounds a row-group request up to a multiple of its 2,048-row vector size, so what actually lands is 51,200 — just above the band, which is why `gpio check optimization`, whose scoring uses the same band as one of its five factors, can mark a freshly sorted file `[fail]` on this factor ([#961](https://github.com/geoparquet/geoparquet-io/issues/961)).
