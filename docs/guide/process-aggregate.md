...
Each pivot holds a count by default. `--breakdown-metric` makes it hold something else, so a category can carry a magnitude rather than a tally:

- `--breakdown month --breakdown-metric "sum:peak"` → `sum_peak_202605`, `sum_peak_202606`, … each the total `peak` for that month.
- Without it, a cell touched once by a strong detection and a cell touched once by a weak one are indistinguishable.

`sum`, `min` and `max` are available; `avg` is not, because a per-category mean cannot be rolled up to a coarser overview level (the output carries no per-category count to weight it by). `--metric avg:<column>` still gives a per-cell mean.
...
| `sum_<col>_<value>` | DOUBLE | Per-category rollups when `--breakdown-metric` is set (`min_`/`max_` likewise); an empty `sum_` bucket reads 0, an empty `min_`/`max_` bucket reads NULL |
| `sum_<col>_other` | DOUBLE | Rollup for categories beyond `--breakdown-limit` |
