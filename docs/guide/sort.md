...
!!! tip "Optimal row group size for spatial queries"
    ...
    **It is not a sort-only default any more.** Until [#981](https://github.com/geoparquet/geoparquet-io/issues/981) the sort commands were the only ones that set a row count: `convert`, `add`, `extract` and `partition` left the choice to their writer, which meant DuckDB's 122,880 on the COPY paths and pyarrow's 100,000 on the Arrow ones - two of gpio's three answers outside the band its own `check optimization` scores. One write facade now resolves the number for every path, so `gpio convert geoparquet in.parquet out.parquet` writes the same 49,152-row groups `gpio sort` does.

    **So does the Python API.** `Table.write()` resolves `row_group_rows` through the same facade, so `gpio.read(...).sort_hilbert().write(out)` is now the equivalent of `gpio sort hilbert in out`, and `write(out, row_group_rows=50000)` snaps to 49,152 exactly as `--row-group-size 50000` does ([#971](https://github.com/geoparquet/geoparquet-io/issues/971)). The explicit `row_group_rows=49152` in the Python examples below is now redundant rather than load-bearing; it is left in because it also documents the number.
