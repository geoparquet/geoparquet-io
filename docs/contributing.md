...
sets `fail_under = 87`, so coverage re-arms the floor on any `--cov` run even
...
- Global floor: `[tool.coverage.report] fail_under` in `pyproject.toml` (87 at the time of writing), measured as combined line+branch coverage.
