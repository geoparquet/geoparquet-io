...
| `ops.create_overviews(input_parquet, levels=None, max_tile_kb=500, bytes_per_cell=None, cell_column=None, scheme=None, output_dir=None, force=False, bands=None, ...)` | Build coarser overview levels from an aggregate file; `bands` states the `level:minzoom` plan explicitly, the same spelling `ops.create_pmtiles_pyramid` takes |
