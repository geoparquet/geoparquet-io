...
| `breakdown_metric` | str | None | What each pivot holds: `None`/`'count'`, or `'sum:col'` / `'min:col'` / `'max:col'` for `<func>_<col>_<value>` columns. `avg` is rejected: a per-category mean cannot be rolled up by `overview` |
