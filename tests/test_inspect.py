...
def test_inspect_head_wkt_output(runner, test_file):
    """Test inspect with --head shows WKT geometry when DuckDB is available."""
    result = runner.invoke(cli, ["inspect", test_file, "--head", "1"])

    assert result.exit_code == 0
    # The output should contain geometry information in the preview
    # Either WKT format (when DuckDB works) or <POINT> fallback
...
def test_wkb_to_wkt():
    """Test WKB to WKT conversion using DuckDB."""
    import struct

    # Build a complete Point WKB
    point_wkb = bytes([0x01, 0x01, 0x00, 0x00, 0x00])  # little endian, Point type
    point_wkb += struct.pack("<d", 1.5)  # x = 1.5
    point_wkb += struct.pack("<d", 2.5)  # y = 2.5

    result = wkb_to_wkt(point_wkb)
    # If DuckDB spatial extension is available, we get WKT
    # If not available, we get None
    if result is not None:
        assert "POINT" in result
        # DuckDB formats as "POINT (1.5 2.5)"
        assert "1.5" in result
        assert "2.5" in result

    # Empty bytes should return None
    assert wkb_to_wkt(b"") is None
    assert wkb_to_wkt(None) is None
