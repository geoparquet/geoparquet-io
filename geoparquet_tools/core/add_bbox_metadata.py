...
from geoparquet_tools.core.common import (
    safe_file_url, get_parquet_metadata, parse_geo_metadata,
    find_primary_geometry_column, check_bbox_structure
)
from geoparquet_tools.core.check_parquet_structure import (
    get_row_group_stats, get_compression_info
)
...
def add_bbox_metadata(parquet_file, verbose=False):
    ...
    try:
        ...
        if os.path.exists(temp_file):
            os.remove(temp_file)
        raise click.ClickException(f"Failed to update metadata: {str(e)}")